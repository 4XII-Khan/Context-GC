...
| **用户偏好** | 写作风格、习惯、纠正、显式偏好 | **完整会话** | 会话中即时检测（5.2）+ 完整会话结束时蒸馏抽取 | 回复更贴合用户 |
...
从对话中抽取的用户相关记忆。**存储形式：一个文件**（如 `preferences.md`），按 user_id 区分。
...
**文件路径**：`{data_dir}/user/{user_id}/preferences.md`（一用户一个偏好文件）

**抽取粒度**：面向**完整会话**，非单轮。抽取时以整场会话的摘要与消息为输入。

**抽取时机**：
- **会话中**（5.2）：`close()` 时轻量级关键词/正则检测显式偏好表达，零 LLM 成本，立即写入
- **会话结束**：蒸馏管道中 `report_factual_content` + `submit_user_preference` 做全面提取与去重
...
- **用户经验目录**：`{data_dir}/user/{user_id}/experience/{任务描述}/`，按任务划分。
...
- **用户经验**：`{data_dir}/user/{user_id}/experience/{任务描述}/`，L0 任务描述 + L1 `.overview.md`
...
└── user/
    └── {user_id}/
        ...
        │   ├── search_code/
        │   └── ...
        └── experience/       # 用户经验，按任务划分
            └── {任务描述}/   # L0：目录名即任务描述
                └── .overview.md   # L1：成功+失败经验一个文件
```
...
| **私有化技能** | 用户下挂技能目录 | `user/{user_id}/skills/{skill_name}/` |
| **用户经验** | 按任务，L0 任务描述 + L1 经验文件 | `user/{user_id}/experience/{任务描述}/.overview.md` |
...
│       ├── skills/                # 私有化技能目录（用户 ID 下挂技能）
│       │   ├── search_code/      # 按技能名分目录，该用户专属
│       │   └── ...
│       └── experience/            # 用户经验目录，按任务划分
│           └── {任务描述}/        # L0：目录名即任务描述
│               └── .overview.md  # L1：成功经验与失败反模式，合在一个文件
└── skills/                        # 公共技能目录，跨用户共享
    ├── search_code/              # 按技能名分目录
    ├── run_shell/
    └── edit_file/
...
class MemoryBackend(Protocol):
    ...
    # 用户经验（按任务划分）
    async def save_user_experience(self, user_id: str, experiences: list[UserExperience], session_id: str) -> None: ...
    async def load_user_experience(self, user_id: str, task_desc: str | None) -> list[UserExperience]: ...
...
class ContextGC:
    ...
    async def close(self) -> None:
        """轮次结束：摘要+分代+合并 + checkpoint 写入（若到达间隔）+ 偏好信号检测（见 5.1/5.2）"""
        ...
    ...
    async def on_session_end(self, user_id: str, agent_id: str, ...) -> None:
        """完整会话结束时：L0/L1/L2 持久化 → 蒸馏管道 → 清理 checkpoint（见 5.3）"""
        ...
```
...
- `flush_distillation`：未注入时使用默认管道（复用 AsMe task_agent、distillation、skill_learner）；注入则可替换为自定义抽取逻辑
...
system = build_system_with_prefs_and_exp(prefs, exps, skills)
```
...
await gc.close()  # 轮次结束：摘要+分代+合并 + checkpoint + 偏好信号检测

# 完整会话结束时：生成 L0/L1/L2、运行蒸馏管道（Task Agent → 蒸馏 → 经验 + Skill Learner）
await gc.on_session_end(user_id, agent_id, generate_l0, flush_distillation=None)

# 新会话开始：加载用户偏好与用户经验
prefs = await gc.get_user_preferences(user_id)
exps = await gc.get_user_experience(user_id)
# 注入到 system 或 prompt，供 LLM 参考
...
**问题**：蒸馏管道在会话结束后才运行，用户在会话中表达的偏好（如"不要用 xxx"、"我喜欢简洁回复"）要等下次会话才生效。Claude Code 和 OpenClaw 均支持会话中即时写入记忆。

**机制**：在 `close()` 中增加**偏好信号检测**——对本轮用户消息做轻量匹配（规则 + 关键词，不调 LLM），检测到显式偏好表达时立即写入 `preferences.md`。

**检测规则**（无 LLM 调用）：

| 模式 | 示例 | 动作 |
|------|------|------|
| 显式纠正 | "不要用 XXX"、"别再这样了" | 写入 `corrections` 类偏好 |
| 显式偏好 | "我偏好 YYY"、"以后都用 ZZZ" | 写入 `explicit_prefs` 类偏好 |
| 风格要求 | "简洁点"、"详细说明"、"用中文回复" | 写入 `writing_style` 类偏好 |

**要点**：
- 仅做关键词/正则匹配，零 LLM 成本
- 误检代价低（偏好可被后续蒸馏覆盖/去重）
- 完整蒸馏仍在 `on_session_end` 运行，会做全面的偏好提取和去重

### 5.3 正常会话结束流程

**每轮 `close()`**：摘要、分代、合并（内存）+ checkpoint 写入（若到达间隔）+ 偏好信号检测。
...
| 组件 | 路径 | 说明 |
|------|------|------|
| 持久化协议 | `context_gc/backend.py` | `MemoryBackend` Protocol，会话/偏好/技能/用户经验存储接口 |
| 持久化后端 | `context_gc/sqlite_backend.py` 或文件后端 | 实现 L0/L1/L2、偏好、技能、经验 |
| Context GC 主类 | `context_gc/context_gc.py` | 会话内压缩 + 注入 backend 时负责持久化与蒸馏触发 |
| Checkpoint 管理 | `context_gc/checkpoint.py` | `.checkpoint.json` 读写、恢复逻辑、raw_messages 增量追加（5.1） |
| 偏好信号检测 | `context_gc/preference_detector.py` | 关键词/正则规则匹配，`close()` 时零 LLM 成本即时抽取偏好（5.2） |
| 记忆生命周期 | `context_gc/lifecycle.py` | 偏好/经验/技能的 TTL 老化、归档、注入容量控制（9.8） |
| 会话过期清理 | `context_gc/session_cleanup.py` | 按 `session_ttl_days` 清理过期会话 L0/L1/L2 及相关文件 |
| 蒸馏管道 | `context_gc/flush.py` 或依赖 AsMe | `flush_distillation`：Task Agent → 蒸馏 → 经验写入 + Skill Learner |
| 宿主示例 | `examples/context_gc_with_storage.py` | 完整调用示例，含 on_session_end、flush_distillation |
...
context_gc:
  ...
  # Checkpoint（崩溃恢复，见 5.1）
  checkpoint_interval: 5          # 每 5 轮写一次 checkpoint；0 = 禁用
  checkpoint_raw_messages: true   # 是否每轮增量追加 raw_messages

  # 蒸馏管道（完整会话结束时）
  learning:
    min_messages_for_learning: 4
    task_agent_max_iterations: 20
    skill_learner_max_iterations: 10
    distillation_max_tokens: 50000  # 蒸馏管道总 token 预算，超限则剩余任务 skip
...
**目录**：`user/{user_id}/experience/{task_desc}/.overview.md`。若同一任务多次出现（如多轮会话），按 **9.5.1 经验去重与冲突处理** 策略写入。

**task_desc 文件系统安全**：长描述可做 slug（如 hash 或截断+sanitize），用索引表维护 `slug ↔ task_desc` 映射。
...
- 对 `task_desc` 做**归一化**再用于目录 Key：关键词抽取（如「登录」「API」）+ 停用词过滤，或短 hash（前 12 位 SHA256）
- 维护 `{data_dir}/user/{user_id}/experience/.task_index.json`：`{ "norm_key": "login-api", "canonical_desc": "实现登录接口", "alt_descs": ["做登录", "login API"] }`
- 写入时：用 `norm_key` 定位目录，`canonical_desc` 作为展示；新 task_desc 若与既有 norm_key 语义相近则复用
- **并发安全**：`.task_index.json` 为单文件，同一用户的多个会话同时结束蒸馏时存在写冲突风险。采用**文件锁**（`fcntl.flock` / 平台适配）保证原子读写；写入时先写临时文件再 `rename`，避免截断损坏。设计上假设**单用户单写者**为主要场景，多写者场景需宿主保证串行化
...
```yaml
experience:
  task_normalize: "keyword" | "hash" | "none"   # 任务描述归一化方式
  dedup_strategy: "exact" | "keyword_overlap" | "llm_similar" | "none"
  dedup_keyword_threshold: 0.8   # keyword_overlap 时重叠率阈值
  conflict_strategy: "append_with_source" | "newer_wins" | "keep_both" | "llm_merge"
```
...
可配置项（参考 AsMe `distillation_config`、`learning`）：

| 配置项 | 说明 | 默认 |
|--------|------|------|
| `min_messages_for_learning` | 最少消息数才触发蒸馏 | 如 4 |
| `task_agent_max_iterations` | Task Agent 最大迭代轮次（每轮 = 一次 LLM 调用 + 工具执行；10 任务会话约需 15–20 轮） | 如 20 |
| `skill_learner_max_iterations` | Skill Learner 最大迭代轮次 | 如 10 |
| `task_agent_system_prompt` | Task Agent system prompt | 复用 AsMe |
| `success_distillation_prompt` | 成功任务蒸馏 prompt | 复用 AsMe |
| `failure_distillation_prompt` | 失败任务蒸馏 prompt | 复用 AsMe |
| `skill_learner_system_prompt` | Skill Learner system prompt | 复用 AsMe |
| `auto_distill_on_session_end` | 会话结束时是否自动触发蒸馏 | true；设为 false 则需宿主显式调用 `flush_distillation` |
| `experience.task_normalize` | 任务描述归一化 | keyword / hash / none |
| `experience.dedup_strategy` | 经验条目去重策略 | exact / keyword_overlap / llm_similar / none |
| `experience.conflict_strategy` | 冲突经验合并策略 | append_with_source / newer_wins / keep_both / llm_merge |

宿主可注入 `flush_distillation` 替换默认管道；未注入时使用默认实现（Task Agent → 蒸馏 → Skill Learner + 经验写入）。
...
| 对比方 | Context GC 的劣势 |
|--------|-------------------|
| **vs Claude Code** | CC 的 Microcompaction（工具大输出落盘）在 Context GC 中无对应；CC 会话中即时写入 Auto memory，Context GC 蒸馏为离线（已增加轻量级会话中偏好抽取缓解） |
...
| ~~**蒸馏纯离线**~~ | ~~会话中偏好无法即时生效~~ | 已设计：[5.2 会话中轻量级偏好抽取](#52-会话中轻量级偏好抽取) |
| ~~**记忆无老化**~~ | ~~偏好/经验/技能只增不减~~ | 已设计：[9.8 记忆老化与淘汰](#98-记忆老化与淘汰) |
| ~~**蒸馏成本未管控**~~ | ~~三阶段管道无预算限制~~ | 已设计：[9.10.2 蒸馏成本估算与预算](#9102-蒸馏成本估算与预算) |

### 10.5 综合结论

方案在**会话内压缩策略**（分代 + 容量触发 + gen_score 衰减）、**会话级记忆形态**（L0/L1/L2 与 GC 摘要映射）、**蒸馏管道设计**（复用 AsMe + 经验/技能双轨 + 成本预算）、**生产可靠性**（checkpoint 崩溃恢复 + 会话中偏好即时抽取）、**记忆生命周期**（老化淘汰 + 注入容量控制）上具有完整设计。**无向量检索**与**嵌入优先**使其在轻量部署场景有差异化。主要不确定性在于分代与蒸馏的实测效果、以及长会话分段下的任务连续性，验证场景见[第十二章](#十二端到端验证场景)。整体处于**设计完备、实现待验**阶段。
...
**generate_summary 可关联强化**（上下文压缩设计 2.5 已有）：摘要须含主题词、实体名、领域术语，便于后续关联度计算。可在 prompt 中显式要求：「输出必须包含可检索关键词（技术栈、文件路径、错误类型等），便于与后续轮次做关联匹配」。
...
| **任务索引一致性** | 写入 experience 前后校验 `.task_index.json`，避免 norm_key 冲突或孤立条目 |
...
|------|------|------|
| 轮次 1–5 | 正常对话 | - |
| 轮次 6 | 用户说"以后用中文回复" | 5.2 偏好信号检测触发，立即写入 preferences.md |
| 轮次 7+ | 继续对话 | 如果宿主在 get_messages 时加载偏好，后续轮次应用中文 |
| **度量** | 轮次 7 的回复语言 | 中文 |
...
- [上下文压缩设计](./上下文压缩设计.md)
- [与 Claude Code 对比](./与ClaudeCode对比.md)
- [与 OpenViking 对比](./与OpenViking对比.md)
- [OpenViking复刻-无向量](./OpenViking复刻-无向量.md)
