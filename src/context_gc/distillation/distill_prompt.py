...
**语言（强制执行）**：通过工具填入的所有字符串字段（含 ``skip_learning`` 的 ``reason``、``facts`` 列表各项等）须使用**简体中文**；必要的代码片段、API 名、路径、专有名词可保留英文。

**领域单一性（影响后续经验与技能落库）**：本次调用产出的所有字段**只描述与本任务描述（task）一致的那一个领域/主题**。不得把**其它任务**、**其它用户目标**或**本会话中无关话题**的教训、流程、事实混写入同一次工具参数（避免被写成**一条**经验或**错误并入**某一技能）。若输入消息里出现多主题，**只提炼与当前任务直接相关**的部分。
...
**语言（强制执行）**：以下各字段内容须使用**简体中文**；必要的代码、标识符、专有名词可保留英文。

**领域单一性**：同上——**仅围绕当前失败任务所属的那一个领域/主题**填写；勿混入无关任务或无关主题的归因与教训，以免经验与技能被错误合并。
...
def format_message_for_distillation(msg: dict) -> str:
    """
    蒸馏输入使用完整消息结构（含 tools/steps 等宿主扩展字段），不截断。
    与 Task Agent 用的 format_message_blob（短摘要）区分。
    """
    try:
        ...
        return json.dumps({"role": role, "content": content}, ensure_ascii=False, indent=2)
...
def pack_distillation_input(
    ...
    if task_messages:
        msg_lines = "\n---\n".join(format_message_for_distillation(m) for m in task_messages)
        parts.append(f"\n## 任务相关消息（完整结构，含工具/步骤等扩展字段）\n{msg_lines}")
